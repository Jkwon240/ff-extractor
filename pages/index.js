...
const FICONS = {"application/pdf":"📄","image/jpeg":"🖼️","image/png":"🖼️","image/webp":"🖼️","text/plain":"📝","text/csv":"📝",
  "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet":"📊","application/vnd.ms-excel":"📊"};
  ...
  const startJob = async (bl) => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (d.status) setJobMeta({ status: d.status || "ongoing", memo: d.memo || "", manager: d.manager || "" });
      }
    } catch {}
    setLoadingJob(false);
  };
  ...
  const saveMeta = async (updates) => {
    ...
    try {
      await fetch("/api/extract", {
        ...
        body: JSON.stringify({ blNo: activeBlNo, ...newMeta }),
      });
    } catch {}
    setSavingMeta(false);
  };
  ...
  const saveFieldEdit = async (fieldKey, value) => {
    try {
      const r = await fetch("/api/extract", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ blNo: activeBlNo, fieldKey, value, source: "직접입력" }),
      });
      const d = await r.json();
      if (d.unified) setUnified(d.unified);
    } catch {}
...
Booking No: ${getFirstVal("booking_no")}`;
  ...
  // Excel export
  const exportExcel = () => {
    const rows = [["BL No", "담당자", "상태", "Shipper", "Consignee", "POL", "POD", "Vessel", "ETD", "ETA", "Container", "Gross Weight", "CBM", "Invoice No", "Invoice Value", "Booking No", "메모"]];
    rows.push([
      activeBlNo, jobMeta.manager, STATUSES.find(s => s.key === jobMeta.status)?.label,
      getFirstVal("shipper"), getFirstVal("consignee"), getFirstVal("port_of_loading"), getFirstVal("port_of_discharge"),
      getFirstVal("vessel"), getFirstVal("etd"), getFirstVal("eta"), getFirstVal("container_no"),
    ...
    a.download = `${activeBlNo}_${new Date().toLocaleDateString('ko-KR').replace(/\./g,'').replace(/ /g,'')}.csv`;
    a.click();
  };

  const filteredList = blList.filter(bl => bl.toLowerCase().includes(search.toLowerCase()));
  const currentStatus = STATUSES.find(s => s.key === jobMeta.status) || STATUSES[0];
            ...
            {isEditing ? (
              <div style={{ display: "flex", gap: 6 }}>
                <input value={editValue} onChange={e => setEditValue(e.target.value)}
                  ...
                  autoFocus />
                <button onClick={() => saveFieldEdit(fieldKey, editValue)}
                  style={{ padding: "5px 12px", background: "#1a1a2e", color: "white", border: "none", borderRadius: 6, cursor: "pointer", fontSize: 12 }}>저장</button>
                <button onClick={() => setEditingField(null)}
                  style={{ padding: "5px 10px", background: "#f5f5f5", border: "none", borderRadius: 6, cursor: "pointer", fontSize: 12 }}>취소</button>
              </div>
            ) : entries.length === 0 ? (
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 13, color: "#ddd" }}>—</span>
                <button onClick={() => { setEditingField(fieldKey); setEditValue(""); }}
                  style={{ fontSize: 11, padding: "1px 8px", borderRadius: 4, border: "1px dashed #ddd", background: "none", cursor: "pointer", color: "#bbb" }}>+ 입력</button>
              </div>
            ) : (
              entries.map((entry, i) => (
                <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 6, marginBottom: entries.length > 1 ? 5 : 0 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <span style={{ fontSize: 11, color: "#aaa", marginLeft: 6, background: "#f5f5f5", borderRadius: 4, padding: "1px 5px" }}>
                      {entry.sources.join(' · ')}
                    </span>
                  </div>
    ...
    <>
      ...
      <div style={{ display: "flex", minHeight: "100vh", fontFamily: "system-ui,sans-serif", background: "#f0f2f5" }}>
        ...
        <div style={{ width: 230, background: "#1a1a2e", color: "white", flexShrink: 0, display: "flex", flexDirection: "column", minHeight: "100vh" }}>
          ...
          {/* Status filter chips */}
          <div style={{ padding: "8px 12px", display: "flex", flexWrap: "wrap", gap: 4, borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
            {STATUSES.map(s => (
              <span key={s.key} style={{ fontSize: 10, padding: "2px 7px", borderRadius: 10, background: "rgba(255,255,255,0.12)", color: "rgba(255,255,255,0.7)", cursor: "pointer" }}>
                {s.label}
              </span>
            ))}
          </div>
          ...
          {/* BL List */}
          <div style={{ padding: "8px 0 4px 16px", fontSize: 10, opacity: 0.4, textTransform: "uppercase", letterSpacing: 1 }}>작업 목록 ({filteredList.length})</div>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                      ...
                      <select value={jobMeta.status} onChange={e => saveMeta({ status: e.target.value })}
                        style={{ padding: "3px 8px", borderRadius: 20, border: "none", fontSize: 12, fontWeight: 700, color: currentStatus.color, background: currentStatus.bg, cursor: "pointer", outline: "none" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={() => setShowTemplate(true)}
                          style={{ padding: "7px 14px", borderRadius: 8, border: "1px solid #ddd", background: "white", color: "#555", fontSize: 12, cursor: "pointer", fontWeight: 600 }}>
                          📋 ERP 템플릿
                        </button>
                        <button onClick={exportExcel}
                          style={{ padding: "7px 14px", borderRadius: 8, border: "none", background: "#2e7d32", color: "white", fontSize: 12, cursor: "pointer", fontWeight: 600 }}>
                          📊 엑셀
                        </button>
